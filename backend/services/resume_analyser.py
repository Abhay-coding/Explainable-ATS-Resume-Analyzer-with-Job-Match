...
from backend.utils.async_helpers import batch_encode_embeddings, EmbeddingCache

logger = logging.getLogger('ats_resume_scorer')

# Global embedding cache (reused across requests)
...
def analyze_full_resume(
    ...
    """
    Optimized resume analysis with parallel Groq parsing and batched embeddings.
    """
    # ✅ STEP 1: Groq parsing (resume + JD)
    logger.info("Starting Groq parsing (resume + JD)...")
    try:
        # Use synchronous parsing - faster and simpler than async
        parsed_resume = parse_resume(resume_text)
        parsed_jd = parse_job_description(job_description) if job_description and job_description.strip() else None
        logger.info("✓ Groq parsing completed")
    except Exception as exc:
        logger.error(f"Groq parsing failed: {exc}")
        raise
    ...
    # ✅ STEP 2: Skill validation with batched embeddings
    logger.info("Validating skills with embeddings...")
    ...
    # ✅ STEP 3: JD comparison (parsed_jd already available from parallel parsing)
    ...
    if parsed_jd:
        ...
        logger.info(f"Comparing resume with JD ({len(jd_keywords)} JD keywords)...")
