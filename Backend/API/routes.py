...
async def analyze_resume(
    ...
    if not nlp:
        raise HTTPException(
            status_code=503,
            detail='spaCy model not loaded yet. Please try again.',
        )
    ...
    if embedder is None:
        try:
            ...
            request.app.state.embedder = SentenceTransformer(SENTENCE_TRANSFORMER_MODEL)
            embedder = request.app.state.embedder
            ...
            raise HTTPException(
                status_code=500,
                detail='Failed to initialize embedder model. Check server logs.',
            )
    ...
    #Full Analysis Pipeline with timeout protection
    try:
        from backend.services.resume_analyser import analyze_full_resume
        from backend.utils.async_helpers import timeout_wrapper
        import asyncio
        
        # Simple timeout wrapper without needing asyncio.run (we're already async)
        ...
        if elapsed > max_timeout:
            logger.error(f'Analysis timeout: Full pipeline exceeded {max_timeout}s (took {elapsed:.1f}s)')
            raise HTTPException(status_code=504, detail=f'Analysis took too long (>{max_timeout}s). Try a smaller resume or disable JD comparison.')
        
        logger.info(f'✓ Full analysis completed in {elapsed:.1f}s')
        
    except TimeoutError as exc:
        logger.error(f'Analysis timeout: {exc}')
        raise HTTPException(status_code=504, detail='Analysis took too long. Try a smaller resume.')
    except Exception as exc:
